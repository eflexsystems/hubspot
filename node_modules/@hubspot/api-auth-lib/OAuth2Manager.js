...
class OAuth2Manager {
  ...
  async fetchAccessToken(exchangeProof) {
    this.logger.debug(
      `Fetching access token for portalId ${this.portalId} for clientId ${
        this.clientId
      }`
    );
    try {
      ...
      if (this.writeTokenInfo) {
        this.logger.debug(
          `Persisting updated tokenInfo for portalId ${
            this.portalId
          } for clientId ${this.clientId}`
        );
  ...
  async exchangeForTokens(exchangeProof) {
    try {
      if (this.refreshTokenRequest) {
        this.logger.debug(
          `Waiting for access token for portalId ${
            this.portalId
          } for clientId ${this.clientId} to be fetched`
        );
